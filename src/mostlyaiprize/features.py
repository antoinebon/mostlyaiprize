...
class SubjectTableEngineer:
    ...
    def __init__(self, subject_column: str, enable_sequential: bool = True, 
                 enable_distribution: bool = True, enable_cross_column: bool = True,
                 enable_entropy: bool = False, enable_correlations: bool = True) -> None:
    ...
    def _compute_sequential_features(self, df: pl.DataFrame, col: str) -> pl.DataFrame:
        ...
        exprs.extend([
            ...
            (pl.col(col).drop_nulls().last() - pl.col(col).drop_nulls().first()).fill_null(0).alias(f"{col}_net_change"),
        ...
        exprs.extend([
            pl.col(col).drop_nulls().slice(0, pl.col(col).drop_nulls().len() // 2).mean().alias(f"{col}_early_half_mean"),
            pl.col(col).drop_nulls().slice(pl.col(col).drop_nulls().len() // 2).mean().alias(f"{col}_late_half_mean"),
        ])
        ...
        if self._enable_distribution:
            exprs.extend([
                ...
                .otherwise(0.0).alias(f"{col}_cv"),
            ])
        
        if self._enable_sequential:
            # Monotonicity checks on non-null values
            exprs.extend([
                pl.col(col).drop_nulls().diff().drop_nulls().min().ge(0).fill_null(True).alias(f"{col}_is_increasing"),
                pl.col(col).drop_nulls().diff().drop_nulls().max().le(0).fill_null(True).alias(f"{col}_is_decreasing"),
            ])
            
            # Direction changes - count sign changes on non-null values
            exprs.append(
                pl.col(col).drop_nulls().diff().drop_nulls().sign().diff().abs().sum().truediv(2).cast(pl.Int64).fill_null(0).alias(f"{col}_direction_changes")
            )
        ...
        exprs.append(
            pl.when(pl.len() > 1).then(
                ...
                (pl.len() * (pl.int_range(pl.len()) ** 2).sum() - 
                 pl.int_range(pl.len()).sum() ** 2)
            ).otherwise(0.0).alias(f"{col}_trend_slope")
        )
    ...
    def _compute_complex_sequential_features(self, df: pl.DataFrame, col: str) -> pl.DataFrame:
        ...
        exprs.append(
            pl.col(col).map_elements(longest_streak, return_dtype=pl.Int64).alias(f"{col}_longest_streak")
        )
        ...
        exprs.append(
            pl.col(col).map_elements(transition_count, return_dtype=pl.Int64).alias(f"{col}_transition_count")
        )
        ...
        exprs.append(
            ...
            .then(
                pl.concat_str([
                    pl.col(col).slice(0, pl.len() - 1).cast(pl.Utf8),
                    pl.lit(","),
                    pl.col(col).slice(1).cast(pl.Utf8)
                ]).mode().first()
            )
        ...
        exprs.append(
            pl.col(col).map_elements(stability_ratio, return_dtype=pl.Float64).alias(f"{col}_stability_ratio")
        )
    ...
    def _compute_correlations(self, df: pl.DataFrame, numerical_cols: list[str]) -> pl.DataFrame:
        ...
        for i, col1 in enumerate(numerical_cols):
            for col2 in numerical_cols[i+1:]:
                exprs.append(
                    pl.corr(pl.col(col1), pl.col(col2)).fill_null(0).alias(f"{col1}_{col2}_correlation")
                )
    ...
    def _compute_cross_column_features(self, df: pl.DataFrame, numerical_cols: list[str]) -> pl.DataFrame:
        ...
        sum_expr = pl.fold(
            ...
            exprs=[pl.col(col) for col in numerical_cols]
        )
    ...
    def create_enhanced_subject_table(self, data: pl.DataFrame) -> pl.DataFrame:
        ...
        subject_table = data.group_by(self._subject_column).agg([
            pl.len().alias("sequence_length")
        ])
        
        # Get column types
        numerical_cols = [col for col, dtype in data.schema.items() 
                         if dtype.is_numeric() and col != self._subject_column]
        categorical_cols = [col for col, dtype in data.schema.items() 
                           if not dtype.is_numeric() and col != self._subject_column]
        ...
        for col in numerical_cols:
            if f"{col}_early_half_mean" in subject_table.columns and f"{col}_late_half_mean" in subject_table.columns:
                subject_table = subject_table.with_columns([
                    (pl.col(f"{col}_late_half_mean") - pl.col(f"{col}_early_half_mean")).alias(f"{col}_early_late_change")
                ])
